      ...
      <div style="padding:80px">
        ...
        <form v-on:submit.prevent="writeContent">
          ...
          <div class="input-container gender">
            ...
            <input type="text" v-model="event.win_num" />
          </div>
          <v-container>
            <v-row>
              <v-col cols="12" lg="6">
                <v-menu ref="menu1" v-model="menu1" :close-on-content-click="false" transition="scale-transition" offset-y max-width="290px" min-width="auto">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field v-model="event.open_date" label="시작일" persistent-hint prepend-icon="mdi-calendar" v-bind="attrs" @blur="date = parseDate(open_date)" v-on="on"></v-text-field>
                  </template>
                  <v-date-picker v-model="event.open_date" @input="menu1 = false"></v-date-picker>
                </v-menu>
              </v-col>
              ...
              <v-col cols="12" lg="6">
                <v-menu v-model="menu2" :close-on-content-click="false" transition="scale-transition" offset-y max-width="290px" min-width="auto">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field v-model="event.end_date" label="마감일" persistent-hint prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                  </template>
                  <v-date-picker v-model="event.end_date" @input="menu2 = false"></v-date-picker>
                </v-menu>
              </v-col>
            </v-row>
          </v-container>
